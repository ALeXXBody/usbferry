"""CLI entrypoint: python3 -m usbferry <command>"""
...
def add_client_opts(p: argparse.ArgumentParser):
    p.add_argument("host", help="usbferry server host")
    p.add_argument("-p", "--port", type=int, default=DEFAULT_PORT)
    p.add_argument("-t", "--token", default=os.environ.get("USBFERRY_TOKEN", ""),
                   help="auth token (or env USBFERRY_TOKEN)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("-V", "--version", action="version", version=f"usbferry {__version__}")
    ap.add_argument("-v", "--verbose", action="store_true")
    sub = ap.add_subparsers(dest="cmd", required=True)

    s = sub.add_parser("serve", help="run the usbferry server (tunnel + web UI)")
...
async def cmd_serve(args):
    ...
    server = UsbferryServer(args.config, overrides)
    await server.start()
    stop = asyncio.Event()
...
async def cmd_connect(args):
    from .client import UsbferryClient
    if args.lan:
...
async def cmd_list_usb(args):
    ...
    async def ready(cli: UsbferryClient):
    ...
    cli = UsbferryClient(args.host, args.port, args.token,
...
async def cmd_attach(args):
    from .client import UsbferryClient, usbip_attach, usbip_detach_ours
...
async def _token_cmd(args, action):
    from .server import UsbferryServer
    server = UsbferryServer(args.config)
    if action == "add":
        token = server.add_token(args.name)
        print(f"token:      {token}")
        print(f"token name: {args.name}")
        print("\nclient usage:")
        print(f"  python3 -m usbferry connect <host> --token {token} --trust --lan")
        print(f"  python3 -m usbferry list-usb <host> --token {token} --trust")
    elif action == "rm":
...
def _fatal() -> None:
    ...
    msg = f"usbferry hit an unexpected error:\n\n{last}\n\n"
    if path:
        msg += f"Full details were saved to:\n{path}\n\n"
    msg += ("If this keeps happening, please open an issue:\n"
            "https://github.com/ALeXXBody/usbferry/issues")
    message_box(msg)
    pause_interactive()
